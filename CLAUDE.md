...
evals/check_citation_match.py      pins the matcher's ARTEFACT tier cold: footnote digits and line-break hyphens pass, different words or numbers do not; --mutate no-artefact|ignore-digits|no-floor|no-letters|empty
...
evals/known_citation_defects.json  the pre-existing record citation defects (103; 118 before the ARTEFACT tier), frozen 2026-09-25 pending an owner decision; check_citations --all diffs against it
tools/check_all.py                 runs every guard from one list, classed cold|needs-pdfs|needs-portfolio; --cold (CI), --list
...
- [~] **Week 6** (from 2026-09-25): spec `docs/superpowers/specs/2026-09-25-week6-landing-design.md`. Sub-project 1 DONE: a digest batch pins its inputs (`manifest.json`; `--check` survives later decisions and separates "inputs moved" from "same inputs, different output"; a batch is never overwritten); `propose_link` writes only `data/proposals/<run_id>.jsonl`; `knowledge_centre_resolve_actor` resolves on EXACT matches only against `data/actor_register.json` (similar names are suggestions, never identity -- measured, 4 of 5 containment matches named the wrong party); `tools/check_all.py` runs every guard, from `scripts/hooks/pre-commit` (enable per clone: `git config core.hooksPath scripts/hooks`) and cold on CI. Record citations were never re-verified after week 1: 118 of 717 in data/records_merged cite the wrong page (75) or are not in the document (43); pinned in evals/known_citation_defects.json pending an owner decision, so the "zero uncited facts" box does not yet hold. 2026-09-25: the shared matcher (`schemas/citation_match.py`) gained an ARTEFACT tier -- same letters in order, every digit run on the page, at least 10 letters -- because a review measured 18 of those 43 "missing" as true quotes hidden by PDF footnote markers and line-break hyphens (the same defect was refusing true quotes in `propose_link` and the review gate); 15 are now on their page, 3 are off_page, and the pinned defect count is now 103 (78 off_page, 25 missing). NEXT: sub-project 2, the publish.
