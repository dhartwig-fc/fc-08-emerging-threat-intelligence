...
governance/publish_boundary.py     what a public page must not carry: local paths, internal file names, email addresses; violations() lists them
evals/check_publish_boundary.py    pins the publish boundary cold: legitimate data/ URLs and metadata prose pass, real paths/internal names/addresses refused; --mutate no-paths|no-internal|no-email
tools/build_walkthrough.py         builds the public walkthrough page (one advisory, PDF to owner-approved links) from governed files only, no clock/git/PDF; --check
site/threat-intel/index.html       the built walkthrough page (tracked); BUILT, NOT YET PUBLISHED -- see the Week 6 status entry and site/PUBLISHED
evals/check_walkthrough.py         pins the walkthrough as a faithful, deterministic projection of its inputs; --mutate drop-citation|unpinned-log|wrong-page|swap-notes|wrong-count|desk-scope|two-runs|typed-score|actor-id|digest-header|telemetry-count|attested-count|swap-moves|proposal-date|desk-total|register-unresolved|corpus-count|emergent-undecided
tools/publish_walkthrough.py       writes the built page into a portfolio checkout's projects/nexus/threat-intel/ and records its sha256 in site/PUBLISHED; refuses a page that differs from a fresh build or crosses the boundary; never commits or pushes either repo, and prints the steps still to do (a publish is not finished when it returns)
evals/check_published_walkthrough.py  the staleness gate: site/PUBLISHED's sha256 must equal the committed page's (cold), and the copy the portfolio has COMMITTED (`git show HEAD:...`, never the file on its disk) must match it (warm); names the missing portfolio add+commit step; NOT RUN (exit 2) when the portfolio is absent or not a git repository; "never published" passes before the first publish; runs inside tools/check_all.py on every commit
evals/check_publisher.py           mutation-verifies tools/publish_walkthrough.py and the gate against TEMPORARY git portfolios only (untracked page FAILS, committed HOLDS), never the real one, never the real site/PUBLISHED; --mutate skip-boundary|skip-freshness|disk-only
...
- [x] **Week 6** (2026-09-25 to 09-26, SLICE 1 CLOSED): spec `docs/superpowers/specs/2026-09-25-week6-landing-design.md`. Sub-project 1 DONE: a digest batch pins its inputs (`manifest.json`; `--check` survives later decisions and separates "inputs moved" from "same inputs, different output"; a batch is never overwritten); `propose_link` writes only `data/proposals/<run_id>.jsonl`; `knowledge_centre_resolve_actor` resolves on EXACT matches only against `data/actor_register.json` (similar names are suggestions, never identity -- measured, 4 of 5 containment matches named the wrong party); `tools/check_all.py` runs every guard, from `scripts/hooks/pre-commit` (enable per clone: `git config core.hooksPath scripts/hooks`) and cold on CI. Record citations were never re-verified after week 1: 118 of 717 in data/records_merged cited the wrong page or text the matcher could not find. Owner decision 2026-09-25 (evals/owner_decisions/citation_repair_2026-09-25.json): the shared matcher (`schemas/citation_match.py`) first gained an ARTEFACT tier (footnote markers, line-break hyphens, NFC-composed letters) and an ELLIPSIS tier (fragments on the page in order), re-measuring the set 118 -> 103 -> 91; a page-spanning tier was tried and dropped, because every tolerance also loosens `propose_link`'s gate. 12 true quotes the matcher cannot place are owner-attested (evals/attested_citations.json). Then in data/records_merged 80 citations were re-paged (75 found on exactly one other page + 5 page-break quotes moved to the page the quote starts on), 4 removed, and the 2 facts left uncited removed (ADV-2026-0004's two indicators); data/records (the extractor's evidence) is untouched. The citation baseline is empty (check_citations --all: 713 checked, 12 verified by attestation, 0 pinned); the current digest batch is slice1-2026-09-25; merged F1 unchanged, typologies 0.704 -> 0.704 (actors 0.692, jurisdictions 0.730, emergent 0.352 -- only unscored indicators were removed). Sub-project 2 DONE and PUBLISHED 2026-09-26 (fc-08 `8626294`, portfolio `22c96bc`; live at https://dhartwig-fc.github.io/dan-hartwig-portfolio/projects/nexus/threat-intel/index.html, byte-identical to the committed page, sha256 `4e28fb8e…` in `site/PUBLISHED`): `tools/build_walkthrough.py` renders the committed `site/threat-intel/index.html` deterministically from governed files (section list and inputs in the file-map row above); `governance/publish_boundary.py` + `evals/check_publish_boundary.py` pin what a public page may never carry (local paths, internal file names, addresses); `tools/publish_walkthrough.py` is the only writer into a portfolio checkout, refuses a page that differs from a fresh build or crosses the boundary, and never commits or pushes either repository; `evals/check_published_walkthrough.py` is the staleness gate (cold half compares `site/PUBLISHED`'s sha256 against the committed page, the warm half compares the copy the portfolio has COMMITTED -- `git show HEAD:`, never its disk; "never published" passes cleanly before the first publish); `evals/check_publisher.py` mutation-verifies the publisher end to end against temporary portfolio directories only. The portfolio edit itself -- `future-capabilities.html`'s Track 2 block moved from "in build" to "Built, slice 1", with a link to the walkthrough -- was merged and pushed with the publish. **The portfolio is shared with other sessions:** on publish day the Tuning Studio session had pushed a week-9 commit touching the same note line minutes earlier, so the branch was rebased onto it (conflict on that one line, resolved keeping both changes). Fetch the portfolio and check `main` against `origin/main` before step 1, every time. `site/PUBLISHED` now exists, so the obligation below is live.

  **fc-08 owns the portfolio's `projects/nexus/threat-intel/`.** Its only writer is `tools/publish_walkthrough.py` and its staleness gate is `evals/check_published_walkthrough.py`; nothing in fc-10 writes or gates it. The checkout is read from `NEXUS_PORTFOLIO` (default `~/Cowork HB/dan-hartwig-portfolio`) -- fc-10 names the same checkout `NEXUS_PORTFOLIO_ROOT`, so setting one does not set the other.

  **The publish procedure, on the owner's go.** A publish is finished when the portfolio has COMMITTED the page and both repositories are pushed -- not when the publisher returns. The site serves what the portfolio pushes; a page left on the portfolio's disk is a live link to a 404 (final review of week 6.2, reproduced on temp clones: the old gate said HOLDS in exactly that state). The order is forced by the pre-commit hook: its `check_published_walkthrough` reads the portfolio's HEAD copy, so it says HOLDS only after step 4, and `site/PUBLISHED` must be committed here after that, never before. Simulated end to end on temp clones of both repositories, the block below run as written with only the paths changed and step 7 not run (`.superpowers/sdd/2026-09-25-week6-2-the-publish/final-fix-report.md`): the gate FAILED after step 3 naming the portfolio commit, a premature commit of `site/PUBLISHED` there was refused by the hook (25 run, 1 failed), and after step 4 the gate said HOLDS and step 6's hook passed (25 run, 0 failed).

  ```bash
  cd ~/fc-08-emerging-threat-intelligence
  export NEXUS_PORTFOLIO="$HOME/Cowork HB/dan-hartwig-portfolio"; P="$NEXUS_PORTFOLIO"
  # 0. the portfolio is shared: fetch, and if origin/main moved, rebase the branch onto it first
git -C "$P" fetch origin && git -C "$P" status -sb
# 1. write the portfolio copy and site/PUBLISHED; it prints the sha256 and the steps below
  .venv/bin/python tools/publish_walkthrough.py --portfolio "$P"
  # 2. look at "$P/projects/nexus/threat-intel/index.html" at 375px and at desktop width (spec Section 10
  #    step 3): no horizontal scroll, the back link reaches future-capabilities.html, the trace links open
  # 3. bring the Track 2 edit onto the portfolio's main (first publish only; if main moved, rebase the branch first)
  git -C "$P" merge --ff-only fc08-slice1-publish
  # 4. commit the page IN THE PORTFOLIO -- the step the old procedure did not have
  git -C "$P" add projects/nexus/threat-intel/index.html && git -C "$P" commit -m "NEXUS: Threat Intelligence walkthrough, published from fc-08"
  # 5. verify: the portfolio's committed copy is the page site/PUBLISHED names
  test "$(git -C "$P" show HEAD:projects/nexus/threat-intel/index.html | shasum -a 256 | cut -d' ' -f1)" = "$(cat site/PUBLISHED)" && echo SAME
  .venv/bin/python evals/check_published_walkthrough.py        # must say HOLDS
  # 6. only now commit site/PUBLISHED here; the hook runs check_all, whose staleness gate HOLDS since step 4
  git add site/PUBLISHED && git commit -m "Week 6.2: publish the walkthrough"
  # 7. push fc-08, then the portfolio; follow the live link
  git push origin main && git -C "$P" push origin main
  ```

  **The obligation this creates, once `site/PUBLISHED` exists:** any later rebuild of the walkthrough page (`tools/build_walkthrough.py`, no `--check`) must be republished (step 1), committed in the portfolio (steps 4-5), and `site/PUBLISHED` committed here TOGETHER WITH the rebuilt page (step 6), BEFORE the next commit to this repo -- `evals/check_published_walkthrough.py` runs inside `tools/check_all.py`, which the pre-commit hook runs on every commit, not only ones touching the walkthrough, so a rebuilt-but-unrepublished page refuses ALL commits until the portfolio's committed copy matches again. The publisher does not refuse an uncommitted page on purpose: on a republish the rebuilt page cannot be committed first (the hook would refuse it, `site/PUBLISHED` still naming the old page), so it only requires the page on disk to equal a fresh build. **A tripwire in the same obligation:** section 10 says no committed extraction had the resolver, and the builder refuses to build once any committed queue holds a proposal dated on or after `RESOLVER_ADDED` (2026-09-25). The builder reads the queue files ON DISK, so the tripwire fires as soon as a new extraction's queue file is written, not when it is committed: from then on `build_walkthrough --check` fails inside `check_all`, and every commit to this repo is refused until section 10's resolver sentence is reworded (and the page republished) in the same commit as that run. Sub-project 3 DONE 2026-09-26: `docs/RELEASE_SLICE1.md` measured all eight definition-of-done boxes (all hold; box 3's golden labels are Claude-drafted and owner-decided only on 15 disputed entries, box 4 carries 12 owner-attested citations, box 6's per-run `terminal_check` has not yet run on a live extraction); tag `fc08-threatintel-slice1-v1.0.0` on `bdfc0f6`; the engineering-journal entry is `06-development-journals/2026/2026-09/2026-09-26_FC08_SLICE1_LANDING.md` in fc_vision_notes (`7085b15`), whose section 4 is the slice-2 list. The LinkedIn draft was dropped by the owner. NEXT: plan slice 2 from that list.
- [x] Week 6: publish slice 1 on `future-capabilities.html` (DONE 2026-09-26), tag `fc08-threatintel-slice1-v1.0.0` (on the commit adding `docs/RELEASE_SLICE1.md`, the definition-of-done audit; all eight boxes hold, box 6 with a stated measurement gap). Journal entry written and pushed (fc_vision_notes `7085b15`).
