...
# 1. write the portfolio copy and site/PUBLISHED; it prints the sha256 and the steps below
  ...
  **The obligation this creates, once `site/PUBLISHED` exists:** any later rebuild of the walkthrough page (`tools/build_walkthrough.py`, no `--check`) must be republished (step 1), committed in the portfolio (steps 4-5), and `site/PUBLISHED` committed here TOGETHER WITH the rebuilt page (step 6), BEFORE the next commit to this repo -- `evals/check_published_walkthrough.py` runs inside `tools/check_all.py`, which the pre-commit hook runs on every commit, not only ones touching the walkthrough, so a rebuilt-but-unrepublished page refuses ALL commits until the portfolio's committed copy matches again. The publisher does not refuse an uncommitted page on purpose: on a republish the rebuilt page cannot be committed first (the hook would refuse it, `site/PUBLISHED` still naming the old page), so it only requires the page on disk to equal a fresh build. **A tripwire in the same obligation:** section 10 says no committed extraction had the resolver, and the builder refuses to build once any committed queue holds a proposal dated on or after `RESOLVER_ADDED` (2026-09-25). The builder reads the queue files ON DISK, so the tripwire fires as soon as a new extraction's queue file is written, not when it is committed: from then on `build_walkthrough --check` fails inside `check_all`, and every commit to this repo is refused until section 10's resolver sentence is reworded (and the page republished) in the same commit as that run. NEXT: sub-project 3, the release (definition-of-done audit, tag `fc08-threatintel-slice1-v1.0.0`, journal entry). The LinkedIn draft was dropped by the owner on 2026-09-26.
- [x] Week 6: publish slice 1 on `future-capabilities.html` (DONE 2026-09-26), tag `fc08-threatintel-slice1-v1.0.0` (on the commit adding `docs/RELEASE_SLICE1.md`, the definition-of-done audit; all eight boxes hold, box 6 with a stated measurement gap). Remaining: the engineering-journal entry in fc_vision_notes.
