...
evals/check_walkthrough.py         pins the walkthrough as a faithful, deterministic projection of its inputs; --mutate drop-citation|unpinned-log|wrong-page|swap-notes|wrong-count|desk-scope|two-runs|typed-score|actor-id|digest-header|telemetry-count|attested-count|swap-moves|proposal-date
tools/publish_walkthrough.py       writes the built page into a portfolio checkout's projects/nexus/threat-intel/ and records its sha256 in site/PUBLISHED; refuses an uncommitted or boundary-crossing page; never commits or pushes either repo
evals/check_published_walkthrough.py  the staleness gate: site/PUBLISHED's sha256 must equal the committed page's (cold), and the portfolio's copy must match it (warm); "never published" passes before the first publish; runs inside tools/check_all.py on every commit
evals/check_publisher.py           mutation-verifies tools/publish_walkthrough.py against TEMPORARY portfolio directories only, never the real one, never the real site/PUBLISHED; --mutate skip-boundary|skip-freshness
